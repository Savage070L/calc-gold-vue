...
export class ActuarialEngine {
  ...
  /**
   * Получить (или построить) коммутационную таблицу для заданного пола.
   * @param {'male'|'female'} gender
   * @param {number} kMult — мультипликатор (по умолчанию 1.0)
   * @param {number} lAdd  — аддитивная поправка (по умолчанию 0.0)
   */
  getCommutationTable(gender, kMult = 1.0, lAdd = 0.0) {
    const key = `${gender}_${this.interestRate}_${kMult}_${lAdd}`;
   ...
   */
  getCITable(gender) {
    const key = `${gender}_${this.interestRate}`;
